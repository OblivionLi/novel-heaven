...
@section('content')
    ...
    <div class="container mainContainer shadow-sm p-3 mb-5 bg-white rounded">
        ...
        <div class="createNovelContent shadow-lg p-3 mb-5 bg-white rounded">
            ...
            <form action="/novels" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    @foreach($genre as $genres)
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="{{ $genres->id }}" name="genres[]"
                                   value="{{ $genres->id }}">
                            <label class="form-check-label" for="{{ $genres->id }}">{{ $genres->genre_name }}</label>
                        </div>
                    @endforeach
                </div>

                <div class="form-group">
                    <a href="/genre-create">Click me to add more genres..</a>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control {{ $errors->has('author') ? ' is-invalid' : '' }}"
                           id="author"
                           name="author" placeholder="Enter author" value="{{ old('author') }}">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control {{ $errors->has('status') ? ' is-invalid' : '' }}"
                           name="status"
                           id="status" placeholder="Enter status" value="{{ old('status') }}">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control {{ $errors->has('translator') ? ' is-invalid' : '' }}"
                           name="translator" id="translator"
                           placeholder="Enter translator" value="{{ old('translator') }}">
                ...
                <div class="form-group">
                    <label for="cover">Novel Picture</label>
                    <input type="file" class="form-control {{ $errors->has('cover') ? ' is-invalid' : '' }}"
                           name="cover"
                           id="cover" placeholder="Enter cover" required>
                    @if ($errors->has('cover'))
                        <span class="invalid-feedback" role="alert">
                ...
                <button type="submit" class="btn btn-outline-success formBtn">Add Novel</button>
            </form>
        </div>
    </div>
